		...
		<div class="box box-info">
            <div class="box-body">
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <select name="lang_value[1]" class="form-control select2">
                            <option value="">Select Currency</option>
                            <?php
                            $statement = $pdo->prepare("SELECT * FROM tbl_currency ORDER BY currency_name ASC");
                            $statement->execute();
                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);   
                            foreach ($result as $row) {
                                ?>
                                <option value="<?php echo $row['currency_name']; ?>" <?php if($row['currency_name'] == $lang_ids[1]){echo 'selected';} ?>><?php echo $row['currency_code']; ?></option>
                                <?php
                            }
                            ?>
                        </select>
